...
export class LambdaStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: DynamodbStreamLambdaCdkStackProps) {
    ...
    this.executeFunction = new lambdaNode.NodejsFunction(this, 'ExecuteFunction', props.executeFunctionLambdaProps); // Lambda関数を作成

    // 信頼ポリシーにEventBridge Schedulerを追加
    (this.executeFunction.role as iam.Role).assumeRolePolicy?.addStatements(new iam.PolicyStatement({
  ...
  this.schedulerControllerFunction = new lambdaNode.NodejsFunction(this, 'SchedulerControllerFunction', {
    ...
    this.schedulerControllerFunction.addToRolePolicy(new iam.PolicyStatement({
      actions: [
