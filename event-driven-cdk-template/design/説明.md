...
3. API Gateway の設定を編集する（必要はない手順を飛ばして OK）
   1. API Gateway を新規作成し、基本設定を編集する
    ...
    ```
    2. リソースを追加し、メソッドを設定する //todo
    ```typescript
        // リソースの作成
        const reservation = api.root.addResource("reservation");
        // メソッドの作成
        reservation.addMethod("POST", new apigateway.LambdaIntegration(props.lambdaProps.createReservation));
    ```
